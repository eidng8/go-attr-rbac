...
func Test_UpdateRole_updates_a_role(t *testing.T) {
	...
	expected := db.Role.Query().Where(role.IDEQ(2)).FirstX(context.Background())
	...
	row := db.Role.Query().Where(role.IDEQ(2), role.NameEQ(name)).
...
func Test_UpdateRole_denies_non_user(t *testing.T) {
	...
	expected := db.Role.Query().Where(role.IDEQ(3)).FirstX(context.Background())
